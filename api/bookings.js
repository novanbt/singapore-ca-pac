...
module.exports = async (req, res) => {
  ...
  const url = req.url || '';
  const queryId = (req.query && req.query.id) || (url.includes('?') ? new URL(url, 'http://localhost').searchParams.get('id') : null);
  ...
  // 3. PATCH / DELETE with ID
  if (queryId) {
    const idx = bookings.findIndex(b => b.id === queryId);
    if (idx === -1) {
      res.statusCode = 404;
